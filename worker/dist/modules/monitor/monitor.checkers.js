...
function parseHostPort(input, defaultPort = 80) {
    let s = input.trim();
...
async function checkHttp(target, now) {
...
async function checkKeyword(target) {
    const started = performance.now();
    let error = null;
    let statusCode = null;
    let found = false;
    try {
        ...
        if (!found)
            error = keyword ? 'keyword not found' : 'no expected value configured';
    }
    catch (e) {
...
async function checkJsonQuery(target) {
    const started = performance.now();
    let error = null;
    let statusCode = null;
    let match = false;
    try {
        ...
        if (!match)
            error = `json query '${target.jsonQuery}' != '${expected}' (got '${String(value)}')`;
    }
    catch (e) {
        error = e.message === 'This operation was aborted' ? 'timeout' : e.message;
    }
...
async function checkDns(target) {
    ...
    const started = performance.now();
    try {
        const res = await withTimeout(Bun.dns.lookup(host), target.timeout * 1000, () => { });
        return {
            status: 'up',
            responseTime: Math.round((performance.now() - started) * 100) / 100,
            statusCode: null,
...
async function checkWebsocket(target) {
    const started = performance.now();
...
async function checkSteam(target) {
...
export async function runCheck(target, now) {
...
async function checkPush(target) {
