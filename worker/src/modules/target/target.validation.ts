...
import type { TargetInput, MonitorType } from './target.types.js'

const MONITOR_TYPES: MonitorType[] = ['http', 'ping', 'tcp', 'dns', 'keyword', 'websocket', 'json-query', 'push', 'steam', 'docker']
const URL_TYPES: MonitorType[] = ['http', 'keyword', 'json-query', 'websocket']
...
export function validateTarget(input: Partial<TargetInput>): TargetInput {
  const name = input.name?.trim()
  const url = input.url?.trim()
  const type: MonitorType = (input.type ?? 'http') as MonitorType

  if (!name) throw HttpError.badRequest('name is required')
  if (!url) throw HttpError.badRequest('url is required')
