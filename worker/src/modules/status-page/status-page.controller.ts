...
const UPTIME_PERIODS = ['24h', '7d', '30d', '90d'] as const
...
router.get('/public/:slug', async (c) => {
  ...
  const heartbeats = Object.fromEntries(
    await Promise.all(
      selected.map(async (m) => [m.id, await MonitorService.heartbeats(Number(m.id), 120)] as const),
    ),
  )
  const uptimes = Object.fromEntries(
    await Promise.all(
      selected.map(async (m) => [
        m.id,
        Object.fromEntries(
          await Promise.all(UPTIME_PERIODS.map(async (p) => [p, await MonitorService.uptime(Number(m.id), p)] as const)),
        ),
      ] as const),
    ),
  )
